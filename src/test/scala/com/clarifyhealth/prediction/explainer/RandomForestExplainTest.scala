...
class RandomForestExplainTest extends QueryTest with SharedSparkSession {

  val ENSEMBLE_TYPE = "rf"

  test("RandomForest regression explain") {
    ...
    val rf_model_path = s"/tmp/${temp_id}"

    val rf_model = model.stages.last.asInstanceOf[RandomForestRegressionModel]
    ...
    val featureImportanceDF = getFeatureImportance(spark, rf_model, predictionDF, featuresColumn)
    ...
    val explainStages = getExplainStages(predictions_view, features_importance_view, labelColumn,
      rf_model_path, ENSEMBLE_TYPE, false)
  ...
  test("RandomForest classification explain") {
    spark.sharedState.cacheManager.clearCache()
    ...
    val rf_model = model.stages.last.asInstanceOf[RandomForestClassificationModel]
    rf_model.write.save(rf_model_path)
    ...
    val explainStages = getExplainStages(predictions_view, features_importance_view, labelColumn,
      rf_model_path, ENSEMBLE_TYPE, true)
