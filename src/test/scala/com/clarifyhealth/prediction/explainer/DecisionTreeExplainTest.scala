...
class DecisionTreeExplainTest extends QueryTest with SharedSparkSession {

  val ENSEMBLE_TYPE = "dct"

  test("DecisionTree regression explain") {
    ...
    val dct_model_path = s"/tmp/${temp_id}"

    val dct_model = model.stages.last.asInstanceOf[DecisionTreeRegressionModel]
    dct_model.write.save(dct_model_path)

    val predictionDF = model.transform(trainDf)
    predictionDF.createOrReplaceTempView(predictions_view)

    val featureImportanceDF = getFeatureImportance(spark, dct_model, predictionDF, featuresColumn)
    ...
    val explainStages = getExplainStages(predictions_view, features_importance_view, labelColumn,
      dct_model_path, ENSEMBLE_TYPE, false)
  ...
  test("DecisionTree classification explain") {
    spark.sharedState.cacheManager.clearCache()
    ...
    val dct_model = model.stages.last.asInstanceOf[DecisionTreeClassificationModel]
    dct_model.write.save(dct_model_path)
    ...
    val explainStages = getExplainStages(predictions_view, features_importance_view, labelColumn,
      dct_model_path, ENSEMBLE_TYPE, true)
